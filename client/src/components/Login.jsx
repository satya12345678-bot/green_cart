...
const onSubmitHandler = async (event) => {
  try {
    ...
    if (data.success) {
     ...
     // window.location.href = "/"; // ✅ no navigate, no NavLink// there was navigate in this line
    } else {
  ...
  return (
    <div  onClick={()=> setShowUserLogin(false)}className="fixed inset-0 z-30 flex items-center justify-center bg-black/50">
      <form  onSubmit={onSubmitHandler} onClick={(e)=>e.stopPropagation()} className="flex flex-col gap-4 p-8 py-10 w-80 sm:w-[352px] text-gray-600 rounded-lg shadow-xl border border-gray-200 bg-white">
